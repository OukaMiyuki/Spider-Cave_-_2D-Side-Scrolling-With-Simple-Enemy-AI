...
public class PlayerMoveController : MonoBehaviour {
    ...
    void modifyPhysics() {
        ...
        bool changeDirectionLinear = (direction.x > 0 && rb.velocity.x < 0) || (direction.x < 0 && rb.velocity.x > 0);
        ...
        if (onGround) { // if character on ground
            ...
            if (Mathf.Abs(direction.x) < 0.4f || changeDirectionLinear) {
            ...
            } else if (rb.velocity.y > 0 && !Input.GetButton("Jump")) { // limit the jump by set the grafity scale when the y velocity above 0 and the player didn't lomg press the jump button
